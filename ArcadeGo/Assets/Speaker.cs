...
namespace LibRetroWrapper
{
    ...
    public class Speaker : MonoBehaviour
    {
        ...
        private float[] _newData = new float[LibRetroWrapper.RetroWrapper.AudioBatchSize];

        private void Start()
        {
            _speaker = GetComponent<AudioSource>();
            if (_speaker == null) return;
            AudioClip clip = AudioClip.Create("Libretro", LibRetroWrapper.RetroWrapper.AudioBatchSize / 2, 2, 44100, true, OnAudioRead);
            _speaker.clip = clip;
            _speaker.Play();
